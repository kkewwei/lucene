...
public class IndexSortSortedNumericDocValuesRangeQuery extends NumericDocValuesRangeQuery {
  ...
  private static class BoundedDocIdSetIterator extends AbstractDocIdSetIterator {
    ...
    public void intoBitSet(int upTo, FixedBitSet bitSet, int offset) throws IOException {
      // Forward to the delegate but never read past lastDoc to preserve the bound semantics.
      // We rely on the delegate's potentially optimized bulk loader (e.g. BitSetIterator,
      // RangeDocIdSetIterator, IndexedDISI.Dense ...).
      assert offset <= doc;
      int boundedUpTo = Math.min(upTo, lastDoc);
      if (boundedUpTo > doc) {
        delegate.intoBitSet(boundedUpTo, bitSet, offset);
      }
      // Re-sync our doc with the delegate's, applying the upper bound.

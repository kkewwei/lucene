...
public class TopDocs {
  ...
  private static class ScoreMergeSortQueue extends PriorityQueue<ShardRef> {
    ...
    public boolean lessThan(ShardRef first, ShardRef second) {
      ...
      if (firstScoreDoc.score < secondScoreDoc.score) {
        ...
        return tieBreakLessThan(first, firstScoreDoc, second, secondScoreDoc, tieBreakerComparator); // 逼平shardId
      }
    }
  }
  ...
  private static TopDocs mergeAux(
    ...
    if (sort == null) {
      queue = new ScoreMergeSortQueue(shardHits, tieBreaker);// 几个shard
    } else {
    ...
    int availHitCount = 0;
    for (int shardIDX = 0; shardIDX < shardHits.length; shardIDX++) {// 遍历每个shard的topHit结果
      ...
      if (shard.scoreDocs != null && shard.scoreDocs.length > 0) {
        ...
        queue.add(new ShardRef(shardIDX));// 每个shard的返回数组，只有score越大的越靠前，其次shardId越小越靠前
      }
    }
    ...
    if (availHitCount <= start) {
      ...
      while (hitUpto < numIterOnHits) { // 要取得个数
        assert queue.size() > 0;
        ShardRef ref = queue.top();  // 每个shard都有一个，这里面保存了每个shard的遍历元数据
        final ScoreDoc hit = shardHits[ref.shardIndex].scoreDocs[ref.hitIndex++];// 通过移动控制变量来遍历

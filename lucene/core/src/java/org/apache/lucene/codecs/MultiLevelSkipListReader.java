...
public abstract class MultiLevelSkipListReader implements Closeable {
  ...
  private int docCount; // 这个词在多少个文档中出现过的
  ...
  private int[] skipInterval;// 每一级别跳表元素记录的相邻跳表的doc 间距。128，1024, 8192...
  ...
  protected int[] skipDoc; // 每个级别的docId
...
// 跳过文档号大于target的第一条文档，返回该文档的docId
  ...
  public int skipTo(int target) throws IOException {
    ...
    while (level < numberOfSkipLevels - 1 && target > skipDoc[level + 1]) {// 直到下一级别的起始文档id高于target
      level++;
    }
...
// 每level之间间隔多少文档数
    ...
    if (level != 0) {
      ...
      childPointer[level] = readChildPointer(skipStream[level]) + skipPointer[level - 1]; // 这一级别跳表在doc的的绝对位置
  ...
  private void loadSkipLevels() throws IOException {
    if (docCount <= skipInterval[0]) {
      ...
      numberOfSkipLevels = 1 + MathUtil.log(docCount / skipInterval[0], skipMultiplier);// 跳表的level(层高)
    }
    ...
    for (int i = numberOfSkipLevels - 1; i > 0; i--) {// 倒序读取每级跳表，可参考 MultiLevelSkipListWriter.writeSkip()
      // the length of the current level
      long length = readLevelLength(skipStream[0]);// 从这个文件首先读取这个级别的文件长度

      // the start pointer of the current level
      skipPointer[i] = skipStream[0].getFilePointer();

      // clone this stream, it is already at the start of the current level
      skipStream[i] = skipStream[0].clone();// 为了读取对应level跳表的值出来

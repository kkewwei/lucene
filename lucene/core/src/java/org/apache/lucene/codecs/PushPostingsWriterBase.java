...
public abstract class PushPostingsWriterBase extends PostingsWriterBase {
   ...
   * only if the term has at least one document. // 还没有开始读取每个词词频时候调用的
   */
  ...
  public void setField(FieldInfo fieldInfo) {// 当前正在写的域
    ...
    writeFreqs = indexOptions.subsumes(IndexOptions.DOCS_AND_FREQS);//确认是否存储了
  ...
  // 处理的是某个field的一个term在每个文档中的词频及startOffset&endOffset，存储到doc(跳表)、pos，pay文件中
  @Override
  public final BlockTermState writeTerm(
    ...
    startTerm(normValues); // 记录doc,pos等文件的起始位置
    ...
    while (true) { // 从内存中的倒排索引获取此term的所有数据，按doc的维度遍历处理该词所在的每个doc的词频及offset
      ...
      docFreq++;
      docsSeen.set(docID); // 在这个文档中可见，主要是统计这个字段（所有term）在多少个文档中出现过
      int freq;
      if (writeFreqs) {///  keyword是不保存词频的，
        freq = postingsEnum.freq(); // 该文档在该词的词频。已经在postingsEnum.nextDoc()中给解析出来了
        totalTermFreq += freq;
      } else {
        freq = -1;
      } // 每扫描一个文档，首先检查上一批block是否处理完了，处理完了则建立调表节点。然后检查是否达到一个block，达到了，则将DocId、freq压缩到doc文件中
      startDoc(docID, freq); // 将跑到 Lucene101PostingsWriter， 会检查上批block是否已经处理完成了。保存freq。只会对doc文件建立跳表节点

      if (writePositions) {// position只是老实存储
        for (int i = 0; i < freq; i++) {// 对每个词的每个position都读取出来
          ...
          if (writeOffsets) {
            ...
            startOffset = -1;
            endOffset = -1;
          } // 检查是否达到一个block的个数（128个词），达到就开始存储DeltaPostion、DeltaStartOffset、length
          addPosition(pos, payload, startOffset, endOffset); // 保存增量的freq。跑到Lucene101PostingsWriter
        }
      }
      //

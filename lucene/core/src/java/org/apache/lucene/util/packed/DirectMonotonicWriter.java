 ...
 * @lucene.internal
 */
public final class DirectMonotonicWriter {// 可以参考http://www.nosqlnotes.com/technotes/searchengine/lucene-docvalues/
 // 使用DirectMonotonicWriter压缩的数据必须是单调递增的
  ...
  // 是对文档Id编号进行了特殊处理，才有最大值，最小值，平均值。每124个value，刷新成一个block
  private void flush() throws IOException { // 在storedField写入时，buffer里面每个元素，都是一个chunk
    assert bufferSize != 0;

    final float avgInc =
        (float) ((double) (buffer[bufferSize - 1] - buffer[0]) / Math.max(1, bufferSize - 1));// 数据的平均值

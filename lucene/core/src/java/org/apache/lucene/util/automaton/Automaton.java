 ...
 * created using {@link #createState}. Mark a state as an accept state using {@link #setAccept}. Add
 * transitions using {@link #addTransition}. Each state must have all of its transitions added at
 * once; if this is too restrictive then use {@link Automaton.Builder} instead. State 0 is always
 * the initial state. Once a state is finished, either because you've starting adding transitions to
 * another state or you call {@link #finishState}, then that states transitions are sorted (first by

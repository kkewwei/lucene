...
public interface IndexableFieldType {
   ...
   */
  boolean storeTermVectors(); // es中对分词的字段有效，主要是term偏移量，词频等信息，默认是关闭的。不能针对不分词的字段设置
...
// es中默认不分词的字段，docValue都是打开的，磁盘存储的，也可以通过"doc_values": false主动关闭。不能针对分词的字段进行设置
  ...
  DocValuesType docValuesType();// 还需要和tokenized配合使用，将分词器关闭才行。默认是打开的
...
// lucene默认是关闭的
   ...
   * point.
   */
  int pointDimensionCount();// 该维度的个数，只要该值不为空，就说明是point存储，有几个维度

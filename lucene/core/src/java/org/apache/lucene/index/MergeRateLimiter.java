...
public class MergeRateLimiter extends RateLimiter {
  ...
  private static final long MIN_PAUSE_NS = TimeUnit.MILLISECONDS.toNanos(2);// 2ms
  private static final long MAX_PAUSE_NS = TimeUnit.MILLISECONDS.toNanos(250);// 250ms

  private volatile double mbPerSec; // 每个线程初始化时不限速
  private volatile long minPauseCheckBytes;// 不超过1mb，也不超过当前写入速度的25ms,就检查一次。

  private AtomicLong lastNS = new AtomicLong(0);// 上次检查符合规范的起始时间
  ...
  public void setMBPerSec(double mbPerSec) {
    ...
    synchronized (this) {
      ...
      this.minPauseCheckBytes =
          Math.min(1024 * 1024, (long) ((MIN_PAUSE_CHECK_MSEC / 1000.0) * mbPerSec * 1024 * 1024));// 最大不超过1M
  ...
  public long pause(long bytes) throws MergePolicy.MergeAbortedException {
    ...
    long paused = 0;
    long delta;
    while ((delta = maybePause(bytes)) >= 0) { // merge暂停累加
      // Keep waiting.
      paused += delta;
    }
   ...
   * OneMergeProgress}. // merge可能会中断    这次检查前，bytes：写了多少数据量，curNS:当前时间
   */
  private long maybePause(long bytes) throws MergePolicy.MergeAbortedException {
    ...
    final double secondsToPause = (bytes / 1024. / 1024.) / rate;// 写入这么多数据，按道理总共的耗时
    ...
    lastNS.updateAndGet(
        last -> {
          ...
          long targetNS = last + (long) (1000000000 * secondsToPause); // 理论写入这些byte后的时间
    ...
    if (curPauseNS > MAX_PAUSE_NS) {
      // 若停顿时间超过250ms。加入合并速度为0，那么最多停顿为MAX_PAUSE_NS，然后写一条，再进来停顿
